  ...
  describe("Joining queue", () => {
    describe("Not already in queue", () => {
      ...
      it("Not on leaderboard | joins queue with 100 MMR and queue time 1 hour from now", async () => {
        ...
        let expectedQueueTime = DateTime.now().plus({ minutes: 60 }).set({ millisecond: 0, second: 0 });
    ...
    it("Already in the queue | updates queue time to 1 hour from now", async () => {
      ...
      let expectedQueueTime1 = DateTime.now().plus({ minutes: 60 }).set({ millisecond: 0, second: 0 });
      ...
      let expectedQueueTime2 = DateTime.now().plus({ minutes: 60 }).set({ millisecond: 0, second: 0 });
